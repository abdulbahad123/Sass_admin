...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <form action="{{ route('whitelabel.website.about.update') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="card-white rounded-2xl p-6 space-y-4">
            <div>
                <label class="block text-xs font-bold text-slate-700 mb-1">About Section Content</label>
                <textarea name="about_content" rows="8" class="w-full bg-slate-50 border border-slate-200 rounded-xl p-4 text-xs leading-relaxed focus:outline-none focus:border-blue-500" placeholder="Describe your agency mission, values, and client approach...">{{ old('about_content', $agency->about_content) }}</textarea>
            </div>
            <div>
                ...
                @if($agency->about_image)
                    <img src="{{ asset($agency->about_image) }}" alt="About" class="h-28 mt-2 rounded-xl object-cover border border-slate-200">
                @endif
            </div>
        </div>
