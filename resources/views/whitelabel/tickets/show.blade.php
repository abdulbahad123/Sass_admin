        ...
        <div class="bg-slate-50/80 rounded-2xl p-5 border border-slate-200/60 text-slate-800 text-xs leading-relaxed space-y-3">
            ...
            @if($ticket->attachment)
                <div class="pt-3 border-t border-slate-200/60">
                    <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">Attached File / Image:</p>
                    @if(Str::endsWith(strtolower($ticket->attachment), ['.jpg', '.jpeg', '.png', '.gif', '.webp']))
                        <a href="{{ $ticket->attachment_url }}" target="_blank" class="inline-block group">
                            ...
                            <span class="text-[11px] font-bold text-blue-600 mt-1 inline-flex items-center">
                    ...
                    @else
                        <a href="{{ $ticket->attachment_url }}" target="_blank" download 
                           class="inline-flex items-center space-x-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-blue-600 hover:bg-blue-50 transition shadow-sm">
                            ...
                            <span>Download File ({{ basename($ticket->attachment) }})</span>
                        </a>
                    @endif
                </div>
            @endif
            ...
            <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pt-2">
                <div class="flex items-center space-x-2">
                    <label class="cursor-pointer px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-xl text-xs font-bold transition inline-flex items-center space-x-1.5">
                        ...
                        <span id="replyFileLabel">Attach Image / File</span>
                        <input type="file" name="attachment" class="hidden" onchange="document.getElementById('replyFileLabel').innerText = this.files[0] ? this.files[0].name : 'Attach Image / File'">
                    </label>
                </div>
