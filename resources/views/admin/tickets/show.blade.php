                ...
                <div class="bg-slate-50/80 rounded-2xl p-5 border border-slate-200/60 text-slate-800 text-xs leading-relaxed space-y-3">
                    ...
                    @if($ticket->attachment)
                        <div class="pt-3 border-t border-slate-200/60">
                            <p class="text-[11px] font-bold uppercase tracking-wider text-slate-400 mb-2">Uploaded Attachment / Image:</p>
                            @if(Str::endsWith(strtolower($ticket->attachment), ['.jpg', '.jpeg', '.png', '.gif', '.webp']))
                                <a href="{{ $ticket->attachment_url }}" target="_blank" class="inline-block group">
                                    <img src="{{ $ticket->attachment_url }}" alt="Attachment" class="max-h-64 rounded-xl border border-slate-200 shadow-sm group-hover:opacity-90 transition">
                                    <span class="text-[11px] font-bold text-indigo-600 mt-1 inline-flex items-center">
                                        <i data-lucide="external-link" class="w-3 h-3 mr-1"></i> View Full Image
                                    </span>
                                </a>
                            @else
                                <a href="{{ $ticket->attachment_url }}" target="_blank" download 
                                   class="inline-flex items-center space-x-2 px-4 py-2 bg-white border border-slate-200 rounded-xl text-xs font-bold text-indigo-600 hover:bg-indigo-50 transition shadow-sm">
                                    ...
                                    <span>Download Attachment ({{ basename($ticket->attachment) }})</span>
                                </a>
                            @endif
                        </div>
                    @endif
                    ...
                    <div class="card-white p-5 rounded-2xl border {{ $reply->is_internal_note ? 'border-amber-200 bg-amber-50/40' : ($isStaff ? 'border-indigo-100 bg-gradient-to-r from-indigo-50/20 to-white' : 'border-slate-100') }} shadow-sm space-y-3">
                        ...
                        @if($reply->attachment)
                            <div class="pl-12 pt-2">
                                @if(Str::endsWith(strtolower($reply->attachment), ['.jpg', '.jpeg', '.png', '.gif', '.webp']))
                                    <a href="{{ $reply->attachment_url }}" target="_blank" class="inline-block group">
                                        <img src="{{ $reply->attachment_url }}" alt="Attachment" class="max-h-48 rounded-xl border border-slate-200 shadow-sm group-hover:opacity-90 transition">
                                    </a>
                                @else
                                    <a href="{{ $reply->attachment_url }}" target="_blank" download 
                                       class="inline-flex items-center space-x-2 px-3 py-1.5 bg-slate-100 border border-slate-200 rounded-lg text-xs font-bold text-indigo-600 hover:bg-indigo-50 transition">
                                        ...
                                        <span>{{ basename($reply->attachment) }}</span>
                                    </a>
                                @endif
                            </div>
                        @endif
            ...
            <div class="card-white p-6 rounded-3xl border border-slate-200/80 shadow-md space-y-4">
                ...
                <form action="{{ route('admin.tickets.reply', $ticket->id) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <!-- Attach File -->
                        <div>
                            <label class="block text-[11px] font-bold uppercase tracking-wider text-slate-500 mb-1">Attachment</label>
                            <label class="cursor-pointer w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 hover:bg-slate-100 transition flex items-center justify-between">
                                <span id="adminReplyFileLabel" class="truncate">Choose image or document</span>
                                <i data-lucide="paperclip" class="w-4 h-4 text-slate-400 shrink-0 ml-1"></i>
                                <input type="file" name="attachment" class="hidden" onchange="document.getElementById('adminReplyFileLabel').innerText = this.files[0] ? this.files[0].name : 'Choose image or document'">
                            </label>
                        </div>
