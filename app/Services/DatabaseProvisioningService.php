...
class DatabaseProvisioningService
{
    ...
    protected function seedTemplateUsersFromMainDb(string $targetDbName): void
    {
        ...
        try {
            $templateUsers = $srcPdo->query("SELECT * FROM users WHERE preview_template = 1 OR template_serial_number IS NOT NULL OR id IN (SELECT DISTINCT user_id FROM user_items WHERE user_id IS NOT NULL) OR username IN ('grocery', 'ecomgrocery', 'manti', 'skinflow', 'fashclo', 'electi', 'furial', 'kidsfa', 'petrashop', 'jewellery', 'clothing', 'fashion', 'furniture', 'pet', 'vegetables')")->fetchAll();
        } catch (\Throwable $e) {
        ...
        $tablesToCopy = [
            ...
            ['table' => 'user_contacts',                    'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_items',                       'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_item_contents',               'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_item_categories',             'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_item_sub_categories',         'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_currencies',                  'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_footers',                     'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_headers',                     'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_menus',                       'fk' => 'user_id', 'rows' => null],
            ['table' => 'user_ulinks',                      'fk' => 'user_id', 'rows' => null],
            ['table' => 'hero_sliders',                     'fk' => 'user_id', 'rows' => null],
            ['table' => 'product_hero_sliders',             'fk' => 'user_id', 'rows' => null],
            ['table' => 'howit_work_sections',              'fk' => 'user_id', 'rows' => null],
            ['table' => 'about_us',                         'fk' => 'user_id', 'rows' => null],
            ['table' => 'about_us_features',                'fk' => 'user_id', 'rows' => null],
